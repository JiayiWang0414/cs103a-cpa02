...
app.get('/comments',
  ...
  async (req,res,next) => {
    try{
      ...
      let items = await Comment.find({userId:userId}); // lookup the user's todo items
      res.locals.items = items;  //make the items available in the view
      res.render("comments");  // render to the toDo page
    } catch (e){
      next(e);
    }
  }
...
//   const {classyear,comments,rating}  =req.body
//   res.locals.classyear= classyear
...
app.post('/comments',
  ...
  async (req,res,next) => {
    try{
      const {classyear,comments,rating,course} = req.body; // get title and description from the body
      const userId = res.locals.user._id; // get the user's id
      let data = {userId,classyear,comments,rating,course} // create the data object
      ...
      res.locals.items= await Comment.find({userId})
      res.render('comments')  // go back to the todo page
    } catch (e){
      next(e);
    }
  }
